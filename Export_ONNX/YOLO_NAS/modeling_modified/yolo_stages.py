...
class YoloNASUpStage(BaseDetectionModule):
    ...
    def forward(self, inputs):
        ...
        x_inter = self.conv(x)
        return x_inter, self.blocks(self.reduce_after_concat(torch.cat([self.upsample(x_inter), *[skip_x1, self.downsample(skip_x2)]], 1)))
...
class YoloNASDownStage(BaseDetectionModule):
    ...
    def forward(self, inputs):
        x, skip_x = inputs
        return self.blocks(torch.cat([self.conv(x), skip_x], 1))
