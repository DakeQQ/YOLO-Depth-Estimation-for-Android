...
//        image_rgb = Process_Texture();  Enable it and disable the line 147~149 for using the NPU + (Yolo v9-s/t) or (NAS)
    ...
    private static void drawBox(LinkedList<Classifier.Recognition> nmsList) {
        ...
        for (Classifier.Recognition draw_target : nmsList) {
            ...
            class_result.append(draw_target.getTitle()).append(" / ").append(String.format("%.1f", 100.f * draw_target.getConfidence())).append("% / ").append(String.format("%.1f", depth_avg)).append(" m").append("\n");
