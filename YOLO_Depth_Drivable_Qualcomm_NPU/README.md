...
您需要从[官方 Qualcomm AI Engine Direct SDK](https://www.qualcomm.com/developer/software/qualcomm-ai-engine-direct-sdk) `（QNN SDK，此演示的版本：2.32.*）`获取额外的库。从官网下载 SDK，并在 `2.32.*/lib/aarch64-android/` & `2.32.*/lib/hexagon-v7*/unsigned/` 目录中找到这些库。确保SDK版本与编译 `libonnxruntime.so` 库时使用的版本一致。
